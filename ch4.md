...
파일 접근 시간과 수정 시간을 변경할 때에는 utime 함수를 사용한다. 
...
이 함수의 행동과 이 함수의 실행에 필요한 권한들은 times 인수가 NULL이냐 아니냐에 따라 다르다. </br>
1. times가 널 포인터이면 접근 시간과 수정 시간 모두 현재 시간으로 설정된다.
2. times가 널 포인터가 아니면 접근 시간과 수정 시간이 times가 가리키는 구조체에 있는 값들로 설정된다.

이 구조체에 상태 변경 시간에 대한 필드가 없음을 주목해야 하는데, 상태 변경 시간은 utime 함수가 호출되면 어차피 자동으로 갱신되기 때문이다. </br>

## 4.20 mkdir 함수와 rmdir 함수

디렉토리를 생성할 때에는 mkdir 함수를 사용함.
        #include <sys/stat.h>

        int mkdir(const char *pathname, mode_t mode);
이 함수는 빈 디렉토리를 새로 만든다. 새 디렉토리에는 도트 디렉토리와 도트-도트 디렉토리에 대한 항목들이 자동으로 만들어진다. mode로 지정된 파일 접근 권한들은 프로세스의 파일 모드 생성 마스크에 의해 수정된다. </br>
이 함수를 사용할 때 디렉토리 안의 파일이름들에 접근하려면 일반적으로 실행 비트들 중 적어도 하나는 켜야 한다. 

빈 디렉토리를 삭제할 때에는 rmdir 함수를 사용한다. 빈 디렉토리는 도트와 도트-도트에대한 항목만을 담은 디렉토리이다. 
        #include <unistd.h>

        int rmdir(const char *pathname);
pathname으로 지정된 디렉토리의 링크 횟수가 이 호출에 의해 0이 되면, 그리고 그 어떤 프로세스도 그 디렉토리를 열어두지 않은 상태이면, 그 디렉토리가 차지하고 있던 공간이 해제된다.

## 4.21 디렉토리 읽기

디렉토리에 대한 읽기 접근 권한을 가지고 있으면 누구라도 디렉토리를 읽을 수 있다. 그러나 디렉토리에 대한 쓰기는 커널만이 가능하다. 이는 파일 시스템의 일관성을 유지하기 위한 것이다. </br>
        #include <dirent.h>
...
telldir 함수와 seekdir 함수는 단일 UNIX 규격의 XSI 확장들이며, 단일 UNIX 규격을 준수하는 모든 UNIX System 구현은 이 함수들을 제공해야 한다. </br>
dirent 구조체의 구체적인 정의는 구현마다 다르지만 다음 두 멤버는 반드시 존재해야 한다.
        struct dirent {
            ino_t d_ino;        /* I노드 번호 */
            char d_name[NAME_MAX + 1];    /* NULL로 끝나는 파일 이름 */
        }
DIR 구조체는 이 여섯 함수들이 현재 읽고 있는 디렉토리에 대한 정보를 유지하는데 사용하는 내부적인 구조체이다. </br>
opendir 함수는 DIR 구조체에 대한 포인터를 돌려준다. 나머지 다섯 함수는 그 포인터를 받는다. opendir 함수는 첫번째 readdir 호출이 디렉토리의 첫 항목을 읽도록 내부적인 정보를 초기화한다. 

## 4.22 chdir, fchdir, getcwd 함수

모든 프로세스는 각자 자신의 현재 작업 디렉토리를 유지한다. 현재 작업 디렉토리는 상대적 경로이름(슬래시로 시작하지 않는 경로이름)의 기준으로 쓰인다. </br>
사용자가 UNIX 시스템에 로그인하면, 보통의 경우 현재 작업 디렉토리는 /etc/passwd 파일의 여섯번째 필드, 즉 사용자의 홈 디렉토리로 시작한다. </br>
현재 디렉토리는 프로세스의 한 특성인 반면 홈 디렉토리는 로그인 이름의 특성이다. </br>
프로세스의 현재 디렉토리는 chdir이나 fchdir 함수로 변경한다.
        #include <unistd.h>

        int chdir(const char *pathname);

        int fchdir(int filedes);
chdir 함수는 경로이름을 받는 반면, fchdir 함수는 열린 파일 서술자를 받는다.

각 디렉토리마다 디렉토리 항목들을 읽어서 방금 전에 거쳐 왔던 디렉토리의 i노드에 해당하는 이름을 찾는다. 이러한 과정을 루트에 도달할 때까지 반복해서 그때까지 찾은 이름들을 조합하면 현재 작업 디렉토리의 전체 경로이름이 나온다.
시스템에서는 getcwd 함수를 제공한다.
        #include <unistd.h>

        char *getcwd(char *buf, size_t size);
buf는 결과가 기록될 버퍼의 주소이고 size는 그 버퍼의 크기이다. 버퍼는 절대 경로이름 + 마지막 널 바이트를 담기에 충분한 크기이어야 하며, 그렇지 않으면 오류가 반환된다.

chdir에 지정한 디렉토리와 getcwd가 알려준 디렉토리는 다르다. 이는 기호 링크의 특성 때문에 비롯된 것이다. chdir 함수는 기호 링크를 따라가는데, getcwd는 디렉토리 트리를 훑어 올라갈 때 /var/spool이 /usr/spool로부터 비롯된 것임을 알지 못한다. getcwd 함수는 응용 프로그램이 작업 도중 현재 작업 디렉토리를 변경하되 프로그램 종료 시에 원래의 현재 작업 디렉토리를 복원하고자 할 때 유용하다. </br>

프로그램 시작 시에 getcwd로 현재 작업 디렉토리의 경로이름을 구해서 보존해 두고, 작업을 마친 후에는 앞에서 보존해 둔 경로이름으로 chdir를 호출해서 원래의 현재 작업 디렉토리로 돌아가면 된다. </br>
fcdir 함수는 getcwd를 호출하는 대신 open으로 현재 디렉토리를 열어서 파일 서술자를 저장해 두고 이후 현재 작업 디렉토리를 복원해야 할 때 그 파일 서술자로 fchdir를 호출하면 된다.

## 4.23 장치 특수 파일

모든 파일 시스템은 자신을 식별하는 주 장치 번호와 부 장치 번호를 가진다. 이 번호들은 dev_t라는 기본 시스템 자료 형식으로 저장된다. </br>
주 번호는 장치 드라이버를 식별하는 용도로 사용되며, 그것과 통신하는 주변 장치 보드를 나타내는 경우도 있다. </br>
부 번호는 특정한 하위 장치를 식별하는데 쓰인다. 

대부분의 구현들은 주 장치 번호와 부 장치 번호에 쉽게 접근할 수 있도록 major와 minor라는 매크로들을 제공한다. 이들을 이용하면 dev_t 객체에 저장된 번호들을 직접 조회할 필요가 없다.

## 4.24 파일 접근 권한 비트 요약

아홉개의 상수들을 세가지 범주로 묶는 것도 가능하다.
